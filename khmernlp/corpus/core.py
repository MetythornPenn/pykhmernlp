...
from khmernlp.corpus import corpus_db_path, corpus_db_url, corpus_path
from khmernlp.tools import get_full_data_path
...
def path_khmernlp_corpus(filename: str) -> str:
    """
    Get path khmernpl.corpus data
...
def get_corpus(filename: str, comments: bool = True) -> frozenset:
    ...
    """
    path = path_khmernlp_corpus(filename)
    lines = []
...
def get_corpus_default_db(name: str, version: str = '') -> Union[str, None]:
    ...
    """
    default_db_path = path_khmernlp_corpus("default_db.json")
