...
def preprocess_phylypo_sample(text: str) -> str:
    ...
    text = text.strip()
    text = text.replace('️', '\u200b')
    # Replace newline characters with space, not necessary if your dataset doesn't contain newline characters
    # text = text.replace('\n', '\u0020')
    text = "".join([c for c in list(text) if c in VOCAB or c == " "])
    text = re.sub('\u0020+', '\u0020', text)
    preprocessed = NON_SPACE_SEP.join(list(text))
...
def text_to_graph_2(text: str, original: str = None):
    """ Convert `text` in a graph representation suitable for training.
    ...
    # Map characters to their corresponding integer indices,
    # use 0 as default value if character not found in CHAR_TO_INT dictionary
    features = np.array([CHAR_TO_INT.get(char, 0) for char in characters])

    print("Features:", features)

    labels = np.array([LABEL_MAP.index(sep) for sep in separators])

    # Convert features and labels arrays to integer type
